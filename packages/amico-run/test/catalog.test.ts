...
describe("matchShape", () => {
  it("exact vetted template match → tier 1", () => {
    ...
    const match = matchShape(
      { platform: "transmon", kind: "gate_synthesis", size: 1 },
      registry,
      exemplars,
      PUBLIC_ALLOW,
    );
  ...
  it("experimental templates are NEVER tier 1 — falls through to the exemplar", () => {
    const { registry, exemplars } = seed();
    const match = matchShape(
      { platform: "rydberg", kind: "gate_synthesis", size: 2 },
      registry,
      exemplars,
      PUBLIC_ALLOW,
    );
  ...
  it("no template and no exemplar → tier 3 (free)", () => {
    const { registry, exemplars } = seed();
    expect(
      matchShape({ platform: "ions", kind: "gate_synthesis", size: 1 }, registry, exemplars, PUBLIC_ALLOW).tier,
    ).toBe("free");
  });
  ...
  it("exemplar match on platform+kind tolerates a size mismatch (near match)", () => {
    const { registry, exemplars } = seed();
    const match = matchShape(
      { platform: "rydberg", kind: "gate_synthesis", size: 3 },
      registry,
      exemplars,
      PUBLIC_ALLOW,
    );
