export const dict = {
  ...
  "desktop.wsl.error.updateVersion": "OpenCodeのアップデートは完了しましたが、{{distro}}から報告されたバージョンはまだ{{installed}}です。想定されるバージョンは{{expected}}です",
  "desktop.wsl.error.noVersion": "バージョンなし",
  "desktop.wsl.error.serverExited": "WSLサーバーが起動後に終了しました (code={{code}} signal={{signal}})",
  "desktop.wsl.error.serverExitedBeforeHealthy": "WSLサーバーが正常に稼働する前に終了しました (code={{code}} signal={{signal}}){{output}}",
  ...
  "help.tabs.sessions": "新しいセッションをタブで開始したり、プロジェクトから既存のセッションを開いたりできます。新しい作業を始めるときは新しいタブを開き、作業が終わったら閉じます。",
  "help.tabs.organize": "複数のタブを開いておくと、進行中のセッションを整理しやすくなります。タブを残しておく場合は、覚えやすい名前に変更できます。",
  "help.tabs.home": "新しいホーム画面には、すべてのセッションとプロジェクトが表示されます。セッションを選択するとタブで開きます。",
  "help.tabs.persistence": "アプリを再度開いたときも、タブは開いたままです。",
  "help.tabs.worktrees": "新しいデザインはまだGitワークツリーに対応していませんが、近日中に対応する予定です。以前のレイアウトを引き続き使用する場合は、設定でレイアウトを切り替えることができます。新しいレイアウトは数週間後に標準となる予定です。",
  ...
  "terminal.connectTicket.csrfError": "PTY接続チケットは、オリジンまたはCSRFチェックによって拒否されました。サーバーのCORS設定を確認してください。",
