...
export const { use: useSettings, provider: SettingsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    // the (now-removed) settings toggle, a stale stored preference, or a
    // prod-channel build default can't strand a user on the legacy design;
    // every amicode surface lives behind this gate. Upstream's transition
    // machinery above stays in place (sunset timers, classification writes,
    // layoutUpgrade), but its verdict no longer drives the gate.
