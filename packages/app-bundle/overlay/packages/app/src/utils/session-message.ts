...
// The stable identity key for a message — its id. Used both as a Binary.search
// key extractor `(message) => message.id` and to produce the search target
// `messageKey(message)`. Overlay-added (not in the pinned upstream base) so the
// server-session store can import it alongside normalizeSessionMessages.
export function messageKey(message: { id: string }): string {
  return message.id
}

// A STABLE, ASCENDING-by-creation-time comparator (earliest first), tie-broken
// deterministically by id. Returns negative when `a` is chronologically older
// than `b`, so `sort(compareMessages)` yields oldest→newest and a reduce that
// keeps the element comparing LESS keeps the OLDEST. The parameter shape is
// structural on purpose: the call sites pass both an SDK `Message`
// (time.created required) and a client `SessionMessageInfo` (.info-shaped, time
// optional on some variants), and this type satisfies both. Overlay-added.
