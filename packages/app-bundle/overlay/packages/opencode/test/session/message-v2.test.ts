...
describe("session.message-v2.toModelMessage — skill parts", () => {
    ...
    const input: SessionV1.WithParts[] = [
      {
        ...
        parts: [
          ...
          },
          {
            ...
            type: "skill",
            name: "transmon",
            description: "Transmon qubit physics",
            content: "# Transmon\n\nUse the Duffing oscillator model.",
          },
        ] as SessionV1.Part[],
      },
    ]
    ...
    expect(await MessageV2.toModelMessages(input, model)).toStrictEqual([
      {
        ...
        content: [
          ...
          { type: "text", text: "# Transmon\n\nUse the Duffing oscillator model." },
        ],
      },
    ])
  })

  test("skill part without content is silently skipped", async () => {
    const input: SessionV1.WithParts[] = [
      {
        info: userInfo("m-skill-no-content"),
        parts: [
          {
            ...basePart("m-skill-no-content", "p1"),
            type: "text",
            text: "hello",
          },
          {
            ...basePart("m-skill-no-content", "p2"),
            type: "skill",
            name: "transmon",
          },
        ] as SessionV1.Part[],
  ...
  test("skill-only message (no text) expands the skill content", async () => {
    const input: SessionV1.WithParts[] = [
      {
        info: userInfo("m-skill-only"),
        parts: [
          {
            ...basePart("m-skill-only", "p1"),
            type: "skill",
            name: "atoms",
            content: "# Rydberg atoms\n\nBlockade model.",
          },
        ] as SessionV1.Part[],
      },
    ]
    ...
    expect(await MessageV2.toModelMessages(input, model)).toStrictEqual([
      {
        role: "user",
        content: [{ type: "text", text: "# Rydberg atoms\n\nBlockade model." }],
      },
    ])
  })
})
