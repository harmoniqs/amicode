...
// A checkout of the private fork turns this from a committed-state integrity
// check into the cross-repository proof: derive the complete delta at the
// manifest's immutable SHA and compare it with the committed tracking copy.
if (existsSync(FORK) && existsSync(join(FORK, ".git"))) {
  const promotion = spawnSync(
    process.execPath,
    [join(PKG_ROOT, "scripts", "overlay-promotion.mjs"), "--check", "--source", FORK, "--revision", manifest.fork_sha],
    { cwd: PKG_ROOT, encoding: "utf8" },
  );
  if (promotion.status !== 0) {
    fail(`cross-repository provenance failed: ${(promotion.stderr || promotion.stdout).trim()}`);
  }
  console.log(`[drift-gate] cross-repository provenance verified at ${manifest.fork_sha.slice(0, 12)}`);
}
