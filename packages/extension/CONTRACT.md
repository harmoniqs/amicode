...
| `result.toml`  | script (atomic)            | Written `result.toml.tmp` then renamed. At least `fidelity` (float) and `iterations` (int); `wall_seconds` — **standing on every bundled emitter** (#711, F-709-1): the solve's own elapsed time (`wall = time() - t0`), pinned by `test/run_dir_emitters.test.ts`; optional on read for pre-standing records.             |
