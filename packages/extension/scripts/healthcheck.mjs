...
function opencodeChecks(boot) {
  if (boot.binMissing) {
    const miss = { ok: false, reason: 'vendored opencode binary missing', fix: 'pnpm --filter amicode-v2 fetch:opencode' }
    return { opencode: miss, creds: { ok: false, reason: 'opencode unavailable (binary missing)', fix: miss.fix } }
  }
  const opencode = boot.eventOk
    ? { ok: true }
    : { ok: false, reason: `vendored opencode did not serve /event 200 (${boot.up ? `status ${boot.eventStatus}` : 'server not up'})`, fix: 'pnpm --filter amicode-v2 fetch:opencode' }
  const creds = boot.signal ?? { ok: false, reason: 'opencode did not boot — creds unverifiable', fix: 'fix opencode boot first' }
