...
function maskedHash(text, beginSrc, endSrc) {
  return (
    "sha256:" +
    createHash("sha256")
      .update(maskFillPoints(text, beginSrc, endSrc))
      .digest("hex")
  );
}
...
for (const entry of readEntries(join(exemplarsDir, "EXEMPLARS.toml"))) {
  ...
  if (!existsSync(scriptPath)) {
    console.error(`build_exemplars: missing in-repo script ${entry.path}`);
    process.exit(1);
  }
  ...
  for (const demo of readdirSync(demosRoot, { withFileTypes: true })) {
    ...
    for (const entry of readEntries(tomlFile)) {
      ...
      if (!existsSync(srcScript)) {
        console.error(`build_exemplars: missing demo script ${demo.name}/${entry.path}`);
        continue;
      }
