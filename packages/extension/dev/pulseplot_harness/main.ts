...
const HARNESS_META = { drives: 2, knots: 50, labels: ["a_1", "a_2"], bounds: [[-0.2, 0.2], [-0.2, 0.2]] as [number, number][] };
...
playBtn.addEventListener("click", () => {
  if (timer) { clearInterval(timer); timer = undefined; playBtn.textContent = "▶ play"; return; }
...
document.getElementById("clear")!.addEventListener("click", () => { plot.clear(); iter = 0; });
...
document.getElementById("bench")!.addEventListener("click", () => {
  ...
  times.sort((a, b) => a - b);
  const med = times[150].toFixed(3), p95 = times[285].toFixed(3), max = times[299].toFixed(3);
...
fetch("./pulse-events.json").then((r) => (r.ok ? r.json() : undefined)).then((events?: Array<Record<string, unknown>>) => {
  if (!events) return;
  ...
  btn.addEventListener("click", () => {
    ...
    timer = setInterval(() => {
      const e = events[i++];
      if (!e) { clearInterval(timer!); timer = undefined; return; }
...
if (location.hash.includes("bench")) setTimeout(() => (document.getElementById("bench") as HTMLButtonElement).click(), 800);
