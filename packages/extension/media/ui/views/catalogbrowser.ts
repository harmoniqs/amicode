...
// row → detail pane. This is the FLAT structure (Krishna leaned flat:
// "iterations w/ flat structure give good history") — one continuous list per
// section, no system/gate grouping. The per-system/gate arrangement is a
// separate variant (#48b) built for the same A/B, not a toggle here.
//
...
export function createCatalogBrowserView(data: CatalogBrowserData): CatalogBrowserView {
  ...
  function section(title: string, rows: HTMLElement[]): HTMLElement {
    ...
    s.append(...(rows.length ? rows : [text("small dim", "none yet").el]));
    return s;
  }

  const qilcRows = data.qilcRuns.map((r) =>
    makeRow(`qilc:${r.run_id}`, [r.tags?.[0], r.gate].filter(Boolean).join(" · ") || r.run_id, r.hash6, {
      kind: "qilc",
      item: r,
    }),
  );
  ...
  list.append(section("QILC runs", qilcRows), section("Results", resultRows));
