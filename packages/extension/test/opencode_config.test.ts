...
describe('buildOpencodeConfigContent', () => {
  ...
  it('honors an explicit pluginPath (the follow-up extension.ts wiring)', () => {
    const cfg = JSON.parse(buildOpencodeConfigContent('/abs/AGENTS.md', TPL, '/elsewhere/amicode_tools.ts'))
  ...
  it('grants external_directory on the entities dir (default + $AMICODE_ENTITIES_DIR override)', () => {
    ...
    try {
      const cfg2 = JSON.parse(buildOpencodeConfigContent('/abs/AGENTS.md', TPL))
