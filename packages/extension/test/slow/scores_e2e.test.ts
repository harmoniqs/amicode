...
describe.skipIf(!existsSync(OC_BIN) || !hasCreds())("scores runtime live e2e (creds required)", () => {
  it(
    "router opens, score #0 interview starts, state pinned + usage funnel recorded",
    { timeout: 300_000 },
    async () => {
      ...
      expect(t1.toLowerCase(), "no stage-batching in turn 1").not.toMatch(
        /max_iter|timestep|objective|constraint|drive_max/,
      );
      ...
      expect(t2.toLowerCase(), "no stage-batching in turn 2").not.toMatch(
        /max_iter|timestep|objective|constraint|drive_max/,
      );
