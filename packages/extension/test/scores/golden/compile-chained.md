...
> Compiled from score `overture` v3 chained into `pulse-designer` v3 — first onboard the user (session zero), then continue straight into pulse design in the SAME session. Sources of truth are the two `SCORE.md` files; do not edit this section by hand.
...
7. **tour** (optional)
   - Q `tour_tabs`: "Session tabs — every chat is a tab" — options: Next
   - Q `tour_new_chat`: "The + button — start a fresh chat" — options: Next
   - Q `tour_sessions`: "Sessions — every chat you've had" — options: Next
   - Q `tour_status`: "Status — how things are running" — options: Next
   - Q `tour_side_panel`: "The side panel — the Pulse Inspector, Preview and your files" — options: Next
   - Q `tour_profile`: "Your profile — the card we just built" — options: Next
   - Q `tour_settings`: "Settings — models, themes, and preferences" — options: Next
   - Q `tour_context`: "The Context panel — what Amico is holding in mind" — options: Next
   - Q `tour_composer`: "The composer — where you talk to Amico" — options: Finish tour
8. **platform**
