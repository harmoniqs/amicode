...
import { pasqalSecretStore } from "./pasqal_secret"
...
const spawnPasqalValidator: PasqalSpawn = async (argv, env) => {
  // PORT SEAM (#451): the fork spawns via Bun.spawn; Node equivalent below —
  // same minimal-env contract, same stdout capture, exit code on resolve.
  return await new Promise((resolve, reject) => {
    const proc = spawn(argv[0], argv.slice(1), {
      env,
      stdio: ["ignore", "pipe", "ignore"], // fixed value-free messages by the #164 contract; stderr not consumed
    })
    let out = ""
    proc.stdout?.on("data", (b: Buffer) => (out += b.toString()))
    proc.once("error", reject)
    proc.once("exit", (code) => resolve({ exitCode: code ?? 0, stdout: out }))
  })
}
...
export async function submitCredentialResponse(rawBody: string, deps: MutationDeps = {}): Promise<string> {
  ...
  // Slack App credentials setup — save client_id + client_secret to ~/.amico/slack-app.json
  if (body.id === "slack-app-credentials" || body.id === "slack-app-client-id") {
    let clientId = ""
    let clientSecret = ""
    if (body.id === "slack-app-credentials") {
      // New flow: token field is JSON { client_id, client_secret }
      try {
        const parsed = JSON.parse(typeof body.token === "string" ? body.token : "")
        clientId = typeof parsed.client_id === "string" ? parsed.client_id.trim() : ""
        clientSecret = typeof parsed.client_secret === "string" ? parsed.client_secret.trim() : ""
      } catch {
        return synthesizeConnection("bad_request", "invalid Slack App credentials payload")
      }
    } else {
      // Legacy flow: token field is the client_id string
      clientId = typeof body.token === "string" ? body.token.trim() : ""
    }
    ...
    try {
      ...
      writeFileSync(join(dir, "slack-app.json"), JSON.stringify(payload, null, 2) + "\n", { mode: 0o600 })
    } catch {
      return synthesizeConnection("write_failed", "could not save Slack App credentials")
    }
...
export async function startAuthResponse(rawBody: string, deps: MutationDeps = {}): Promise<string> {
  ...
  if (id === "slack") {
    ...
    const AMICODE_SLACK_CLIENT_SECRET = (() => {
      const env = process.env.AMICODE_SLACK_CLIENT_SECRET?.trim();
      if (env) return env;
      try {
        const appFile = join(homedir(), ".amico", "slack-app.json");
        const data = JSON.parse(readFileSync(appFile, "utf8"));
      ...
      try {
        ...
        const tokenRes = await fetch("https://slack.com/api/oauth.v2.access", {
          ...
          body: new URLSearchParams({
            ...
            client_secret: AMICODE_SLACK_CLIENT_SECRET,
            code,
    ...
    try {
      ...
      try { const child = spawn(opener, [authorizeUrl], { stdio: "ignore", detached: true }); child.unref() } catch {}
    } catch {}
  ...
  try {
    ...
    if (browserCmd) {
      ...
      // PORT SEAM (#451): the fork uses the `open` npm package; the extension
      // host opens via the platform opener instead — same best-effort contract.
      const opener = process.platform === "darwin" ? "open" : "xdg-open"
      try {
        const child = spawn(opener, [pendingUrl], { stdio: "ignore", detached: true })
        child.unref()
      } catch {}
    }
  } catch {}
