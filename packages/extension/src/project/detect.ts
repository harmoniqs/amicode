...
// A Research Project is identified by a `project.toml` at its root.
 ...
 * A Research Project has a `project.toml` manifest at its root.
 ...
 * `project.toml` after initial registration updates its type on next
 * resolution.
 */
export function detectProjectType(dir: string): ProjectType {
  try {
    return existsSync(join(dir, "project.toml")) ? "research" : "dev";
  } catch {
    return "dev";
  }
}
